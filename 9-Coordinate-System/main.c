...
void prepareDraw(GLint programID,GLint VBO,GLint EBO,GLuint textureID1,GLuint textureID2)
{
	...
	hmm_mat4 view = HMM_Mat4d(1.0f);
	
	
	hmm_mat4 rotate = HMM_Rotate(-55.0f, HMM_Vec3(1.0f, 0.0f, 0.0f));
	model = HMM_MultiplyMat4(model, rotate);
	
	hmm_mat4 trans =  HMM_Translate(HMM_Vec3(0.0f, 0.0f, -3.0f));
	view = HMM_MultiplyMat4(view, trans);
