...
void prepareDraw(GLint programID,GLint VBO,GLint EBO,GLuint textureID1,GLuint textureID2)
{
	...
	hmm_mat4 trans =  HMM_Translate(HMM_Vec3(0.5f, -0.5f, 0.0f));
	hmm_mat4 rotate = HMM_Rotate(counter, HMM_Vec3(0.0f, 0.0f, 1.0f));
	hmm_mat4 model = HMM_MultiplyMat4(trans, rotate);
	unsigned int transformLoc = glGetUniformLocation(programID, "uTransform");
	glUniformMatrix4fv(transformLoc, 1, GL_FALSE, (const GLfloat*)model.Elements);
...
int SDL_main(int argc, char **argv)
{
    ...
    while (isRunning())
    {
		prepareDraw(programID, VBO,EBO,textureID1,textureID2);
		draw();
    }
