...
Camera camera(HMM_Vec3(0.0f, 0.0f, 6.0f));
...
void draw(GLint programID)
{
	...
	hmm_mat4 view = camera.GetViewMatrix();
	
	hmm_mat4 projection = HMM_Perspective(camera.Zoom, (float)WinWidth/WinHeight, 0.1f, 100.0f);
	 ...
	 switch (evt.type)
	 {
		  case SDL_MOUSEMOTION:
		  {
			if (SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT))
			{
				...
				float xoffset = evt.motion.x - lastX;
				float yoffset = lastY - evt.motion.y; // reversed since y-coordinates go from bottom to top
