...
GLint  createShader(const char * shaderName,GLenum shaderType, const char * defines)
{
	...
	const char * array2[] = {defines,shaderSource};
	const char * array1[] = {shaderSource};

	GLint shaderObjectID = glCreateShader(shaderType);
	if(defines != NULL)
	{
	glShaderSource(shaderObjectID, 2, array2, NULL);
	}
	else
	{
	glShaderSource(shaderObjectID, 1, array1, NULL);
	}
...
int SDL_main(int argc, char **argv)
{
	...
	GLint fragmentShaderID = 	createFragmentShader("FragmentShader.glsl", "#define RED_COLOR");
